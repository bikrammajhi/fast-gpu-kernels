            ...
            TC      * C, CStride dC, CSmemLayout          , TiledMma mma)
{
    ...
    // Main K-tile loop with cp.async
    int k_tile_count = size<3>(tAgA);
    for (int k_tile = 0; k_tile < k_tile_count; ++k_tile) {
        // Asynchronous copy gmem -> smem (bypasses L1, writes directly to smem)
        ...
        // Ensure all threads finish reading smem before next iteration overwrites it
        __syncthreads();
    }
          ...
          cudaStream_t stream = 0)
{
    ...
    // Swizzled shared memory layout (128B swizzle)
    ...
    auto sB = tile_to_shape(swizzled_128B_atom, make_shape(bN, bK));
