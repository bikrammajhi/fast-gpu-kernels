...
```bash
nvcc -O3 -arch=sm_80 -lcublas -I<your cutlass>/include -I<your cutlass>/tools/util/include \
    -o /tmp/bench benchmark.cu
ncu --set full --warp-sampling-interval auto --clock-control base \
    --launch-skip 1 --launch-count 1 -o out/ptx_gemm.ncu-rep /tmp/bench
```
...
```bash
modal run scripts/ncu_capture.py --src kernels/cute/A100/benchmark.cu --gpu A100-40GB
