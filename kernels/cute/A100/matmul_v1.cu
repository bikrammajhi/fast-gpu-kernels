          ...
          cudaStream_t stream = 0)
{
    ...
    // Plain (non-swizzled) shared memory layouts
    // K-contiguous row-major so LDSM reads consecutive addresses
    auto sA = make_layout(make_shape(bM, bK), make_stride(bK, Int<1>{})); // (BLK_M,BLK_K) row-major
    auto sB = make_layout(make_shape(bN, bK), make_stride(bK, Int<1>{})); // (BLK_N,BLK_K) row-major
    auto sC = make_layout(make_shape(bM, bN));                             // (BLK_M,BLK_N) (unused)

    // Gmem -> smem copy (DefaultCopy, always correct)
    using GmemCopyAtom = DefaultCopy;
