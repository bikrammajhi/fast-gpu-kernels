...
// =============================================================================
// matmul_v11 — More warps for better ILP and latency hiding
//   v11a: 128x128x64, 4x2 warps (256 threads) — more warps in M dim
//   v11b: 128x128x64, 4x4 warps (512 threads) — maximum warps
//   v11c: 128x128x32, 4x2 warps, 4-stage — smaller k + deeper pipeline
// =============================================================================

template <int CTA_SIZE, int HEIGHT, int WIDTH>
__device__ static void g2s_v11(
...
__global__ void matmul_v11_kern(
    ...
    auto load_AB = [&](int k_iter) {
        ...
        g2s_v11<CTA_SIZE, BLOCK_M, BLOCK_K>(A_ptr, K, A_shm_base + stage_id * AB_size, tid);
        g2s_v11<CTA_SIZE, BLOCK_N, BLOCK_K>(B_ptr, K, B_shm_base + stage_id * AB_size, tid);
        A_ptr += BLOCK_K;
        B_ptr += BLOCK_K;
...
// v11a: 128x128x64, 4x2 warps (256 threads), 2-stage
inline void matmul_v11a_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                                __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=128, BN=128, BK=64, WM=4, WN=2, ST=2;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v11_kern<BM,BN,BK,WM,WN,ST>,
        cdiv(M,BM)*cdiv(N,BN), WM*WN*WARP_SIZE, smem, A,B,C,M,N,K);
}

// v11b: 128x128x64, 4x4 warps (512 threads), 2-stage
inline void matmul_v11b_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                                __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=128, BN=128, BK=64, WM=4, WN=4, ST=2;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v11_kern<BM,BN,BK,WM,WN,ST>,
        cdiv(M,BM)*cdiv(N,BN), WM*WN*WARP_SIZE, smem, A,B,C,M,N,K);
}

// v11c: 128x128x32, 4x2 warps (256 threads), 4-stage
inline void matmul_v11c_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                                __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=128, BN=128, BK=32, WM=4, WN=2, ST=4;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v11_kern<BM,BN,BK,WM,WN,ST>,
        cdiv(M,BM)*cdiv(N,BN), WM*WN*WARP_SIZE, smem, A,B,C,M,N,K);
}

// v11d: 128x128x64, 4x2 warps (256 threads), 3-stage
inline void matmul_v11d_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                                __nv_bfloat16* C, int M, int N, int K) {
...
// v11e: 256x128x64, 4x2 warps (256 threads), 2-stage
inline void matmul_v11e_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                                __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=256, BN=128, BK=64, WM=4, WN=2, ST=2;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v11_kern<BM,BN,BK,WM,WN,ST>,
        cdiv(M,BM)*cdiv(N,BN), WM*WN*WARP_SIZE, smem, A,B,C,M,N,K);
}

// v11f: 256x128x64, 4x4 warps (512 threads), 2-stage
inline void matmul_v11f_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                                __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=256, BN=128, BK=64, WM=4, WN=4, ST=2;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v11_kern<BM,BN,BK,WM,WN,ST>,
        cdiv(M,BM)*cdiv(N,BN), WM*WN*WARP_SIZE, smem, A,B,C,M,N,K);
}

// v11g: 128x256x64, 4x4 warps (512 threads), 2-stage
inline void matmul_v11g_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                                __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=128, BN=256, BK=64, WM=4, WN=4, ST=2;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v11_kern<BM,BN,BK,WM,WN,ST>,
        cdiv(M,BM)*cdiv(N,BN), WM*WN*WARP_SIZE, smem, A,B,C,M,N,K);
}
