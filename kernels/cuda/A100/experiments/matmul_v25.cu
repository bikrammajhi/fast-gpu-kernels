...
// =============================================================================
// matmul_v25 — 4-stage pipeline on standard 128x128x64 tile
//   From v11a baseline, only change is NUM_STAGES=4.
//   Tests whether deeper pipeline helps on A100.
// =============================================================================
...
__global__ void matmul_v25_kern(
...
// v25: 128x128x64, 4x2 warps (256 threads), 4-stage pipeline
inline void matmul_v25_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                              __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=128, BN=128, BK=64, WM=4, WN=2, ST=4;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v25_kern<BM,BN,BK,WM,WN,ST>,
