...
// =============================================================================
// matmul_v22 — v11a baseline with cp.async.ca only (2-stage, same tile)
//   Isolates the cache-always policy change from the 3-stage regression in v21.
// =============================================================================
...
__global__ void matmul_v22_kern(
...
// v22: 128x128x64, 4x2 warps (256 threads), 2-stage, cp.async.ca only
inline void matmul_v22_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                              __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=128, BN=128, BK=64, WM=4, WN=2, ST=2;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v22_kern<BM,BN,BK,WM,WN,ST>,
