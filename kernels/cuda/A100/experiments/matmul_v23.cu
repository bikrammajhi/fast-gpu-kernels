...
// =============================================================================
// matmul_v23 — v11a baseline with 3-stage pipeline only (cp.async.cg, same tile)
//   Isolates deeper pipeline from v21's combined .ca + 3-stage regression.
//   Uses existing g2s_swizzled / cp_async (.cg) helpers, ST=3.
// =============================================================================
...
__global__ void matmul_v23_kern(
...
// v23: 128x128x64, 4x2 warps (256 threads), 3-stage, cp.async.cg only
inline void matmul_v23_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                              __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=128, BN=128, BK=64, WM=4, WN=2, ST=3;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v23_kern<BM,BN,BK,WM,WN,ST>,
