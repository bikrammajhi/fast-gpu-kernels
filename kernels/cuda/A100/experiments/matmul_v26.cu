...
// =============================================================================
// matmul_v26 — 2D tiled CTA scheduling for L2 reuse (A100-compatible)
//   From fast.cu's matmul_6 example: Schedule<1> maps blockIdx to (m,n)
//   in 2D tiles of TM×TN blocks. This keeps adjacent CTAs spatially close,
//   improving B-tile L2 reuse across CTA launches without cluster barriers.
// =============================================================================

template <
    int BLOCK_M, int BLOCK_N, int BLOCK_K,
    int NUM_WARP_M, int NUM_WARP_N,
    int NUM_STAGES, int TM, int TN
>
__launch_bounds__(NUM_WARP_M * NUM_WARP_N * WARP_SIZE)
__global__ void matmul_v26_kern(
    ...
    bool has_next = true;
    int num = schedule_it * gridDim.x + bid;
    while (has_next) {
        int cur_tile = num / (TM * TN);
        int cur_tile_pos = num % (TM * TN);
        num_block_m = TM * (cur_tile / (grid_n / TN));
        num_block_n = TN * (cur_tile % (grid_n / TN));
        num_block_m += cur_tile_pos / TN;
        num_block_n += cur_tile_pos % TN;
        if (num_block_m < grid_m && num_block_n < grid_n) {
            has_next = false;
        } else {
            schedule_it++;
            num = schedule_it * gridDim.x + bid;
            if (num >= grid_m * grid_n) {
                return;
            }
        }
    }

    A += num_block_m * BLOCK_M * K;
    B += num_block_n * BLOCK_N * K;
    C += (num_block_m * BLOCK_M + warp_id_m * WARP_M) * N + (num_block_n * BLOCK_N + warp_id_n * WARP_N);
...
// v26a: 128x128x64, 2-stage, 2D tile[16x8]
inline void matmul_v26a_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                               __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=128, BN=128, BK=64, WM=4, WN=2, ST=2, TM=16, TN=8;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v26_kern<BM,BN,BK,WM,WN,ST,TM,TN>,
        cdiv(M,BM)*cdiv(N,BN), WM*WN*WARP_SIZE, smem, A,B,C,M,N,K);
}

// v26b: 128x128x64, 2-stage, 2D tile[8x8]
inline void matmul_v26b_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                               __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=128, BN=128, BK=64, WM=4, WN=2, ST=2, TM=8, TN=8;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v26_kern<BM,BN,BK,WM,WN,ST,TM,TN>,
        cdiv(M,BM)*cdiv(N,BN), WM*WN*WARP_SIZE, smem, A,B,C,M,N,K);
}
