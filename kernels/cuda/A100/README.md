...
Hand-optimised BF16 GEMM on NVIDIA A100-SXM4-40GB.

- **Hardware:** A100-SXM4-40GB, 312 TFLOPS BF16 peak, 108 SMs, 1.5 TB/s HBM2e
- **Shape:** M = N = K = 16384, bf16
- **Best kernel:** 258.7 TFLOPS (82.9% of cuBLAS at 300.4 TFLOPS)
...
|--------|-----------|--------|--------|---|
| v1 | Baseline (sync loads, 2×2 warps, single-stage) | 64.2 | 20.6% | — |
...
Full optimization journal (per-version breakdowns, diagrams, lessons learned): [docs/README.md](docs/README.md)
...
```bash
nvcc -O3 -arch=sm_80 -lcublas -o /tmp/bench benchmark.cu
