...
// =============================================================================
// matmul_v12 — Double-buffered register loads to overlap ldmatrix with MMA
//   Key insight: while MMA executes for k-slice N, simultaneously load
//   registers for k-slice N+1 via ldmatrix. This hides ldmatrix latency.
//   Uses 2 separate register buffers and alternates between them.
// =============================================================================

template <int CTA_SIZE, int HEIGHT, int WIDTH>
__device__ static void g2s_v12(
...
__global__ void matmul_v12_kern(
    ...
    auto load_AB = [&](int k_iter) {
        ...
        g2s_v12<CTA_SIZE, BLOCK_M, BLOCK_K>(A_ptr, K, A_shm_base + stage_id * AB_size, tid);
        g2s_v12<CTA_SIZE, BLOCK_N, BLOCK_K>(B_ptr, K, B_shm_base + stage_id * AB_size, tid);
        A_ptr += BLOCK_K;
        B_ptr += BLOCK_K;
        cp_async_commit_group();
    };

    // Load registers for a given k-slice from shared memory
    auto load_regs_for_slice = [&](int k, int stage_id, uint32_t A_out[][4], uint32_t B_out[][2]) {
        ...
        for (int m = 0; m < NUM_MMA_M; m++) {
            ...
            LDMATRIX_X4(A_out[m], A_addr ^ (k * 32));
        }
        #pragma unroll
        for (int n = 0; n < NUM_MMA_N; n += 2) {
            uint32_t B_addr = B_shm_thread + stage_offset;
            B_addr += n * MMA_N * BLOCK_K * (int)sizeof(__nv_bfloat16);
            LDMATRIX_X4(B_out[n], B_addr ^ (k * 32));
        }
    };

    // Execute MMA for a given k-slice
    auto mma_slice = [&](int k, uint32_t A_in[][4], uint32_t B_in[][2]) {
        #pragma unroll
        for (int m = 0; m < NUM_MMA_M; m++)
            #pragma unroll
            for (int n = 0; n < NUM_MMA_N; n++)
                MMA_M16N8K16(A_in[m], B_in[n], acc[m][n]);
    };

    // Double-buffered compute: load k+1 regs while computing k
    auto compute_double_buffered = [&](int k_iter) {
        const int stage_id = k_iter % NUM_STAGES;
        uint32_t A_buf[2][NUM_MMA_M][4];
        uint32_t B_buf[2][NUM_MMA_N][2];

        // Load first k-slice
        load_regs_for_slice(0, stage_id, A_buf[0], B_buf[0]);
        ...
        for (int k = 0; k < NUM_MMA_K; k++) {
            ...
            // Load next k-slice regs (if exists) while we prepare to compute current
            if (k + 1 < NUM_MMA_K)
                load_regs_for_slice(k + 1, stage_id, A_buf[nxt], B_buf[nxt]);

            // Compute current k-slice
            mma_slice(k, A_buf[cur], B_buf[cur]);
        }
    };
    ...
    for (int k_iter = 0; k_iter < num_k_iters - PD; k_iter++) {
        ...
        __syncthreads();
        compute_double_buffered(k_iter);
    }
...
// v12: 128x128x64, 4x2 warps (256 threads), 2-stage, double-buffered regs
inline void matmul_v12_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                               __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=128, BN=128, BK=64, WM=4, WN=2, ST=2;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v12_kern<BM,BN,BK,WM,WN,ST>,
        cdiv(M,BM)*cdiv(N,BN), WM*WN*WARP_SIZE, smem, A,B,C,M,N,K);
}

// v12b: 128x128x64, 2x2 warps (128 threads), 2-stage, double-buffered regs
inline void matmul_v12b_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                                __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=128, BN=128, BK=64, WM=2, WN=2, ST=2;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v12_kern<BM,BN,BK,WM,WN,ST>,
        cdiv(M,BM)*cdiv(N,BN), WM*WN*WARP_SIZE, smem, A,B,C,M,N,K);
}
