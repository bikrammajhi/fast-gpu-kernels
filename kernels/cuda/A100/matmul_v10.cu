...
// =============================================================================
// matmul_v10 — Interleaved ldmatrix + MMA per k-slice
//   Key insight: load A[k], B[k], then immediately MMA for k, instead of
//   loading ALL k-slices first. Reduces register lifetime and improves ILP.
//   Also tries 2-pass accumulator split for better instruction scheduling.
// =============================================================================

template <int CTA_SIZE, int HEIGHT, int WIDTH>
__device__ static void g2s_v10(
...
__global__ void matmul_v10_kern(
    ...
    auto load_AB = [&](int k_iter) {
        ...
        g2s_v10<CTA_SIZE, BLOCK_M, BLOCK_K>(A, K, A_shm_base + stage_id * AB_size, tid);
        g2s_v10<CTA_SIZE, BLOCK_N, BLOCK_K>(B, K, B_shm_base + stage_id * AB_size, tid);
        A += BLOCK_K;
        B += BLOCK_K;
        cp_async_commit_group();
    };

    // === INTERLEAVED: load regs + MMA per k-slice ===
    // For each k-slice: load A[k], B[k], then MMA for that k.
    // This keeps only 1 k-slice of A/B in registers at a time.
    auto compute_interleaved = [&](int k_iter) {
        const int stage_id = k_iter % NUM_STAGES;
        const uint32_t stage_offset = stage_id * AB_size;

        // Temporary per-k-slice register buffers
        uint32_t A_reg[4];
        uint32_t B_reg[2];
        ...
        for (int k = 0; k < NUM_MMA_K; k++) {
            ...
            for (int n = 0; n < NUM_MMA_N; n += 2) {
                // Load B[k][n] and B[k][n+1] together (ldmatrix_x4)
                uint32_t B_addr = B_shm_thread + stage_offset;
                B_addr += n * MMA_N * BLOCK_K * (int)sizeof(__nv_bfloat16);
                uint32_t B_pair[4];
                LDMATRIX_X4(B_pair, B_addr ^ (k * 32));

                #pragma unroll
                for (int m = 0; m < NUM_MMA_M; m++) {
                    // Load A[k][m] for each (m,n) pair — compiler should CSE this
                    uint32_t A_addr = A_shm_thread + stage_offset;
                    A_addr += m * MMA_M * BLOCK_K * (int)sizeof(__nv_bfloat16);
                    uint32_t A_r[4];
                    LDMATRIX_X4(A_r, A_addr ^ (k * 32));

                    // Split MMA into two halves for B pair
                    MMA_M16N8K16(A_r, &B_pair[0], acc[m][n]);
                    if (n + 1 < NUM_MMA_N)
                        MMA_M16N8K16(A_r, &B_pair[2], acc[m][n+1]);
                }
            }
        }
    };

    // === STANDARD: load all regs then compute ===
    auto compute_standard = [&](int k_iter) {
...
// =============================================================================
// Launch: 128x128x64, 2x2 warps, 2-stage (best known config)
// =============================================================================
inline void matmul_v10_launch(const __nv_bfloat16* A, const __nv_bfloat16* B,
                               __nv_bfloat16* C, int M, int N, int K) {
    constexpr int BM=128, BN=128, BK=64, WM=2, WN=2, ST=2;
    constexpr int smem = (BM*BK + BN*BK) * (int)sizeof(__nv_bfloat16) * ST;
    launch_safe(matmul_v10_kern<BM,BN,BK,WM,WN,ST>,
