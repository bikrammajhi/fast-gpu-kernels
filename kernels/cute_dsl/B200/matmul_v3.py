...
def benchmark(callable, a_tensor, b_tensor, c_tensor):
    ...
    print(f"  A ({a_tensor.shape})  [{a_tensor.dtype}]")
    print(f"  B ({b_tensor.shape})  [{b_tensor.dtype}]")
    print(f"  C ({c_tensor.shape})  [{c_tensor.dtype}]")
...
kernel = cute.compile(host_function, a_tensor_, b_tensor_, c_tensor_, kernel)

# Run the kernel
benchmark(kernel, a_tensor_, b_tensor_, c_tensor_)
